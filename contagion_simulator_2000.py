...
def runsimulation(idmatfull,industrymat,statemat,countymat,sizemat,aggmatrix,positions,sizerow,idrow):
    ...
    while mortalitynumber < 2000:
        ...
        while len(propogation) > 0:
            ...
            affected = choose_affected(idmatfull, statemat, industrymat, countymat, bnode, aggmatrix, positions, idrow,
                                       sizerow)
            paffected = createdeepcopy(affected)
            newaffected = applymarkovchain(affected, mortalitynumber, idmatfull, statemat, sizemat)
            statemat = applynewstates(bnode, newaffected, idmatfull, statemat)
            statemat, mortalitynumber, propogation = checknewbankruptcy(paffected, newaffected, statemat,
                                                                        mortalitynumber,
                                                                        propogation)


    return mortalitynumber, times