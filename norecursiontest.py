...
def applymarkovchain(affected,mortalitynumber,idmatfull,statemat,fullsizemat,lower,higher):
    newaffected = []
    ...
    for i in affected:
        node =[]
        node.append(i[0])
        node.append(MarkovChain.setState(i[1],fullsizemat[i[0]-1],mortalityrate,lower,higher))
...
def choose_affected(idmatfull,statemat,node,aggmatrix,positions,idmat,sizemat):
    ...
    for i in range(len(countysector)):
        ...
        node = [idmatfull[Id-1],statemat[Id-1]]
        affected.append(node)

    return affected

def runsimulation(idmatfull,industrymat,statemat,countymat,sizemat,aggmatrix,positions,sizerow,idrow,lower,higher):
    mortalitynumber = 0
    while mortalitynumber < 1000:
        ...
        newaffected = applymarkovchain(affected,mortalitynumber,idmatfull,statemat,sizemat,lower,higher)
        statemat = applynewstates(bnode, newaffected,idmatfull,statemat)
        propogation = []
        statemat,mortalitynumber,propogation = checknewbankruptcy(paffected, newaffected,statemat,mortalitynumber,propogation)
        while len(propogation) > 0:
